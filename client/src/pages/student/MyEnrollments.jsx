...
const {enrolledCourses, calculateCourseDuration, navigate, backendUrl, getToken, fetchUserEnrolledCourses} = useContext(AppContext)
  ...
  return (
    ...
    <div className='md:px-36 px-8 pt-10'>
      ...
      <table className='md:table-auto table-fixed w-full overflow-hidden border mt-10'>
        <thead className='test-gray-900 border-b border-gray-500/20 text-sm text-left max-sm:hidden'>
        <tr>
              ...
              <tr key={index} className='border-b border-gray-500/20'>
                ...
                <td className='px-4 py-3 max-sm:text-right'>
                  <button className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white' onClick={()=> navigate('/player/' + course._id)}>
                    {progressArray[index] && progressArray[index].lectureCompleted === progressArray[index].totalLectures && progressArray[index].totalLectures > 0
                      ? 'Completed'
                      : 'On Going'}
                    </button>
                </td>
